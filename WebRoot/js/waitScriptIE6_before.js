...
//设置默认属性
	...
	_initBox : function() {
		...
		//		var xx = this.box.cloneNode();
		////		frame[0].document.body.insertBefore(xx,
		////				frame[0].document.body.childNodes[0]);
		//		//		frame[1].document.body.insertBefore(xx,
		//		//				frame[1].document.body.childNodes[0]);
		//		frame[2].document.body.insertBefore(xx,
		//				frame[2].document.body.childNodes[0]);
		//for ( var i = 0; i < frame.length; i++) {
		//alert(frame[i].document.body.childNodes[0]);

		//	frame[i].document.body.insertBefore(xx,
		//			frame[i].document.body.childNodes[0]);
		//}
	...
	show : function(isResize) {
		...
		for ( var i = 1; i < frame.length; i++) {
			var xx = frame[i].document.body.childNodes[0];
...
var RepairFixed = function() {
	...
	"remove" : function(elem) {
		...
		if (elem.parentNode === layer) {
			...
			elem[parent] ? elem[parent].appendChild(elem) : layer
					.removeChild(elem);
			elem.removeAttribute(parent);//不能用delete
		}
		//没有内部元素就移除
		if (!layer.childNodes.length && layer.parentNode == body)
...
if ($$B.ie6) {
	AlertBox.prototype._initialize = (function() {
		var init = AlertBox.prototype._initialize, methods = {
			"init" : function() {
			...
			"show" : function(isResize) {
				...
				if (this.fixSelect) {
					if (!this._iframe) {
						this._iframe = this.box
								.appendChild(document
										.createElement("<ifr"
												+ "ame style=\"position:absolute;z-index:-1;filter:alpha(opacity=0.8);\">"));
						isResize = true;
					}
					if (isResize) {
						...
						$$D.setStyle(this._iframe, {
							...
							width : size.width + "px",
							top : -this.box.clientTop + "px",
							left : -this.box.clientLeft + "px"
						});
					}
				}
			},
			...
			"dispose" : function() {
				RepairFixed.remove(this.box);
				if (this._iframe)
					this.box.removeChild(this._iframe);
				this._iframe = null;
			}
		};
		return function() {
		...
		"show" : function(isResize) {
			if (this.center) {
				...
				if (isResize) {
					var size = $$D.getSize(this.box);
					$$D.setStyle(this.box, {
						marginTop : (this.fixed ? 0 : $$D.getScrollTop())
								- size.height / 2 + "px",
						marginLeft : (this.fixed ? 0 : $$D.getScrollLeft())
								- size.width / 2 + "px",
						top : "50%",
						left : "50%"
					});
				...
				if (this._centerCss) {
					$$D.setStyle(this.box, this._centerCss);
...
var OverLay = function() {
	var overlay;
	function Create() {
		var lay = document.body.insertBefore(document.createElement("div"),
