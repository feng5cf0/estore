...
function checkSubmit() {
	...
	var isUploaded = document.getElementById("uploaded");
...
function uploadFile() {
	var winFeatures = 'dialogHeight:400px;dialogWidth:600px;center:yes;resizable=no';

	if (Length(uploadFile.value) > 50) {
		alert("版本名称长度过长，请重新输入！！！");
		return false;
	}

	if (!isNumberAndAlpha(uploadFile.value)) {
		alert("文件路径只能是数字或字母！！！");
		uploadFile.focus();
		return;
	}
	...
	<body>
		<form action="${basePath}core/checkstandVersionUpdate!update.action"
				...
				<div class="roadbody1">
					...
					<div class="divtable">
						<table width="100%" border="0" cellspacing="1" cellpadding="0"
							bgcolor="#cecece">
							<input type="hidden" value="${checkstandVersion.id}"
								name="checkstandVersion.id" />
							<tr>
								<td class="tableitem1">
									项目名称
								</td>
							...
							<tr>
								...
								<td class="tableitem2">
									<input type="text" id="checkstandVersion.versionName"
										...
										class="dmg-input" value="${checkstandVersion.versionName}" />
									<FONT color=red>*</FONT>
								</td>
							...
							<tr>
								...
								<td class="tableitem3">
									<input type="hidden" id="checkstandVersion.filePath"
										name="checkstandVersion.filePath"
										value="${checkstandVersion.filePath}" />
