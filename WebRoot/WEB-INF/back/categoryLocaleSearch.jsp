...
window.parent.categoryLocaleTree.location.reload();
</script>
</c:if>
<script type="text/javascript">
...
function setAvailable(id, upRightId, page) {
	...
	function addItem(categoryId) {
		window.location.href= "${basePath}back/categoryLocaleAction!prepare.action?categoryId="+categoryId;
	}
						...
						<div class="divbutton2">
							<ld:check mark="rightInsert">
								<input type="button" value="新增" class="button2"
									...
									onmouseout="this.className='button2Out'" onclick="addItem(${categoryId});" />
							</ld:check>
						</div>
						<div class="divbutton2">
					...
					<div class="divtable">
						<table id="tb" width="100%" border="0" cellspacing="1"
							...
							<c:forEach items="${categoryList}" var="item" varStatus="status">
								...
								<td><c:out value="${item.localeType}" /></td>
								<td><c:out value="${item.isAvaliable}" /></td>
									<td>
